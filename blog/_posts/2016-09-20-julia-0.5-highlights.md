...
As another fun example of filtering, let's find first names that are palindromes – the same spelled forward and backward. If you want to follow along, download [this list of common names](/blog/data/first-names.txt). The list contains all first names reported by the US government as having been given to at least a thousand babies since 1880, sorted by descending popularity. First, load the names:
