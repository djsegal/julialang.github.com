...
As you can see, the functional version using `map` is over 22 times slower than the vectorized form, which uses specialized generated code. Next, the same comparison in Julia 0.5:
...
The version defined in terms of `map` is now as fast (maybe even faster) than using a vectorized method. Now, in this case, writing `2v` happens to be more convenient than writing `map(x->2x, v)` – and you can keep using vectorized forms when they're more convenient. However, there are many cases where functional constructs are more general, clearer, or more convenient, and now using them is also efficient. The result is that Julia APIs everywhere can be made simultaneously, more general, more elegant, and more concise, all without sacrificing any speed.
...
You may have noticed that the return type here is `T`, which is a type parameter of the `clip` method. Not only is that allowed, but the return type can be an arbitrary expression of argument values, type parameters, and values from outer scopes. For example:
