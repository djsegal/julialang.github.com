...
Julia's array comprehensions have always supported some advanced features such as iterating with multiple variables to produce multidimensional arrays. This release rounds out the feature set of comprehensions, adding two additional features: nested generation with multiple `for` clauses and filtering with a following `if` clause. To demonstrate these features, consider making a dollar (100¢) using quarters (25¢), dimes (10¢), nickels (5¢) and pennies (1¢). We can generate a list of the total value amounts of each kind of coin using a comprehension with nested for clauses:
...
Each tuple is the value in quarters, dimes, nickels and pennies, so each tuple should add up to 100:

    julia> julia> extrema(sum(t) for t in ans)
    (100,100)

I'm using a generator to pass the sum of each tuple to the `extrema` reducer without creating an intermediate array. Excellent. So now we know there are 242 ways to make a dollar with common coins. Suppose we want to ensure that the value in pennies is less than the value in nickels, and so forth. By adding a filter clause, we can do this too:
...
There are exactly four ways to do this. Who knew? As another fun example of filtering, let's find first names that are palindromes – the same spelled forward and backward. If you want to follow along, download [this list of names](/blog/data/first-names.txt). First, load the names:
...
Of nearly five thousand first names, only eighteen are palindromes – and all of them are female! That is some hard-hitting data science.
