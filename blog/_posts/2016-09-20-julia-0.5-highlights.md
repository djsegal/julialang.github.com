...
Comprehension filtering allows the `for` clause of a comprehension to be followed by an `if` clause, so that the comprehension only produces values where the predicate is true. For example, here we find palindromes by loading the dictionary found on most UNIX systems (incling OS X/macOS) and filtering words that are equal to their reverse:

    julia> words = map(chomp, open(readlines, "/usr/share/dict/words"))
    ...
    julia> [w for w in words if length(w) > 1 && w == reverse(w)]
