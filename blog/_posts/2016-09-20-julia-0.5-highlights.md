...
As another fun example of filtering, let's find first names that are palindromes – the same spelled forward and backward. If you want to follow along, download [this list of names](/blog/data/first-names.txt). First, load the names:
...
Now, we can use a filtered comprehension to find the ones that are equal to themselves reversed, ignoring case:
