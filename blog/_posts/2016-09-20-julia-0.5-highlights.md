...
That's a solid improvement, but much larger improvemnts can be found, especially when using the kinds of highly efficient, custom representations of data for which Julia has become well known. For example, suppose we only care about small indices from 1 to 64; then we can make a thin wrapper around a 64-bit unsigned integer which looks and feels like an `IntSet`:

    immutable SmallIntSet <: Base.AbstractSet{Int}
        bits::UInt64
        function SmallIntSet(itr)
            bits = zero(UInt64)
            for k in itr
                1 <= k <= 64 || error("value out of range: $k")
                bits |= one(UInt64) << ((k - 1) % UInt64)
            end
            return new(bits)
        end
    end

    function Base.show(io::IO, s::SmallIntSet)
        print(io, "SmallIntSet([")
        k, bits = 0, s.bits
        while bits > 0
            t = trailing_zeros(bits) + 1
            k += t
            print(io, k)
            bits >>= t
            bits > 0 && print(io, ", ")
...
Previous versions of Julia have special syntax for dictionay comprehensions:
...
While this syntax is convenient, it fails to generalize beyond the built-in `Dict` type. In languages where built-in types like `Dict` are truly special, this special syntax may be just fine, but in Julia `Dict` is just a normal user-defined type that happens to be defined in the standard library. What if you want to construct a `SortedDict` object in a similar fashion, or some other user-defined type? In 0.5, we can instead construct a `Dict` by passing it a generator of `Pair` objects:
...
Unlike the previous special syntax, this generalizes to other associative collection types. Suppose, for example,
