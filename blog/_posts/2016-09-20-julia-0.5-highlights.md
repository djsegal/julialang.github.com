...
In timing examples above, we used an array comprehension to time an expression many times, producing an array of elapsed times, to which we applied the `mean` function to get an average elapsed time. But we don't save the array of elapsed times, and the mean can easily be computed from streamed data. In Julia 0.5 if you leave the `[` and `]` around an array comprehension out, you get a *generator expression* instead, which can be iterated over, yielding one value at a time, and many standard library functions can work with arbitrary iterable collections, including generators. Thus, we could have computed the mean elapsed times like so instead:

    julia> mean(@elapsed(double_it_vec(v)) for _=1:100)
    0.023680031440000013
    
    julia> mean(@elapsed(double_it_map(v)) for _=1:100)
    0.022724940119999995

This avoids allocating a temporary array of elapsed times, instead passing a generator expression to the `mean` function, which takes a value at a time, maintaining their sum and count as it goes, returning their mean at the end. Many funtions in Julia's standard library can take generators as arguments. Suppose, for example, we'd like to know the standard deviation of elapsed times, instead:

    julia> std([@elapsed(double_it_vec(v)) for _=1:100])
    0.0002903552274711975
