...
The total value in quarters is `q`, value in dimes is `d`, etc. We increment `q` from zero to 100 in increments of 25, `d` from zero to `100-q` in increments of ten, and so on. Thus, each tuple `(q,d,n,p)` is a breakdown of value into quarters, dimes, nickels and pennies. Note that the iteration for `p` isn't a range at all, it's a single number, `100-q-d-n`. This value guarantees that each tuple adds up to a dollar:

    julia> extrema(sum(t) for t in ans)
    (100,100)

Note that expression uses a generator to pass the sum of each tuple to the `extrema` function without creating an intermediate array of sums. Slick, right? So now we know there are 242 ways to make a dollar with common coins. But suppose we want to ensure that the value in pennies is less than the value in nickels, and so forth. By adding a filter clause, we can do this easily too:
...
The only difference is the `if p < n < d < q` at the end of the comprehension. It turns out that there are exactly four ways break a dollar this way. Who knew?

As another fun example of filtering, let's find first names that are palindromes – the same spelled forward and backward. If you want to follow along, download [this list of common names](/blog/data/first-names.txt). Specifically this list contains all first names reported by the US government as having been given to 1000 or more babies since 1880, sorted by descending popularity. First, load the names:

    julia> names = map(chomp, open(readlines, "first-names.txt"))
    10247-element Array{String,1}:
     "James"
     "John"
     "Robert"
     "Michael"
...
The top of the list is predominantly male, while the bottom is more female, reflecting the fact that boys are more often given common names, whereas girls are more likely to be given uncommon names. With this data loaded, we can use a filtered comprehension to find the ones that are equal to themselves reversed, ignoring case:
...
Of over 10,000 first names, only 20 are palindromes (0.2%), – and only three of those 20 are boys' names (15%). Who knew? Now that is some hard-hitting data science.

Nested and filtered comprehensions aren't earth-shattering features – everything you can do with them can be done other ways – but they are expressive and convenient, and they allow you to try more things with your data quickly and easily, with less tedious refactoring. With the introduction of generators and the addition of nesting and filtering, the Julia story for comprehensions is now quite complete.
