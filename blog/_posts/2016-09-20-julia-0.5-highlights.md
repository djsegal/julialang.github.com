...
Julia's array comprehensions have always supported some advanced features such as iterating with multiple variables to produce multidimensional arrays. This release rounds out the feature set of comprehensions, adding two additional features: filtering with the `if` keyword and nested generation with multiple uses of the `for` keyword.

Comprehension filtering allows the `for` clause of a comprehension to be followed by an `if` clause, so that the comprehension only produces values where the predicate is true. For example, let's find first names that are palindromes – the same spelled forward and backward. If you want to follow along, download [this list of names](/blog/data/first-names.txt). First, load the names:
...
One of the features Julia's comprehensions have always had, that is not often found in other languages with is the ability to generate multidimensional arrays using comprehensions. We can, for example, generate a matrix of small rationals like so:

    julia> [i//j for i = 1:6, j = 1:6]
    6×6 Array{Rational{Int64},2}:
     1//1  1//2  1//3  1//4  1//5  1//6
     2//1  1//1  2//3  1//2  2//5  1//3
     3//1  3//2  1//1  3//4  3//5  1//2
     4//1  2//1  4//3  1//1  4//5  2//3
     5//1  5//2  5//3  5//4  1//1  5//6
     6//1  3//1  2//1  3//2  6//5  1//1

Suppose, that instead of this matrix, we want all of the values in the lower-triangular part of `H₅` as a vector. One way to do it on 0.5 is to use a nested comprehension:
